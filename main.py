...
parser.add_argument('--vocab_size', type=int, default=320, help='vocab size')
...
parser.add_argument('--validate_every', type=int, default=5000, help='train iterations')
parser.add_argument('--validate_for', type=int, default=200, help='validate iterations')
    ...
    if config['train']:
        ...
        for i, (batch, sample, character_mismatch, accuracy) in enumerate(out):
            ...
            for reactant, product, single_sample in zip(reactants[:, 1:], products[:, 1:], sample[:, :-1]):
                i = (reactant == val_dataset.token_encoder['<eos>']).nonzero(as_tuple=False)[0]
                r_smi = ''.join([val_dataset.token_decoder[react] for react in reactant[:i]])
                dump_data['reactants'].append(r_smi)
                
                i = (single_sample == val_dataset.token_encoder['<eos>']).nonzero(as_tuple=False)[0] if val_dataset.token_encoder['<eor>'] in single_sample else single_sample.size(0)
