...
from x_transformers.lora import Linear, Embedding
...
class AbsolutePositionalEmbedding(nn.Module):
    def __init__(self, dim, max_seq_len, l2norm_embed = False, num_layers = 1):
        ...
        self.l2norm_embed = l2norm_embed
        self.emb = nn.ModuleList(Embedding(max_seq_len, dim) for _ in range(num_layers))
...
class DynamicPositionBias(nn.Module):
    def __init__(self, dim, *, heads, depth, log_distance = False, norm = False):
        ...
        self.mlp.append(Linear(dim, heads))
...
class FeedForward(nn.Module):
    def __init__(
        ...
        self.ff = nn.Sequential(
            ...
            nn.Dropout(dropout),
            Linear(inner_dim, dim_out, bias = not no_bias)
        )
...
class Attention(nn.Module):
    def __init__(
        ...
        self.to_q = Linear(dim, q_dim, bias = False)
        self.to_k = Linear(dim, k_dim, bias = False)

        # shared key / values, for further memory savings during inference
        assert not (shared_kv and value_dim_head != dim_head), 'key and value head dimensions must be equal for shared key / values'
        self.to_v = Linear(dim, v_dim, bias = False) if not shared_kv else None

        # relations projection from tp-attention
        self.to_r = Linear(dim, v_dim, bias = False) if tensor_product else None

        # add GLU gating for aggregated values, from alphafold2
        self.to_v_gate = None
        if gate_values:
            self.to_v_gate = Linear(dim, out_dim)
        ...
        self.to_out = nn.Sequential(Linear(out_dim, dim * 2, bias = False), nn.GLU()) if on_attn else Linear(out_dim, dim, bias = False)
...
class ViTransformerWrapper(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.mlp_head = Linear(dim, num_classes) if exists(num_classes) else nn.Identity()
...
class TransformerWrapper(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.project_emb = Linear(emb_dim, dim) if emb_dim != dim else nn.Identity()
        self.attn_layers = attn_layers

        self.init_()

        logits_dim = default(logits_dim, num_tokens)
        self.to_logits = Linear(dim, logits_dim) if not tie_embedding else lambda t: t @ self.token_emb.emb.weight.t()
...
class ContinuousTransformerWrapper(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.project_in = Linear(dim_in, dim) if exists(dim_in) else nn.Identity()

        self.attn_layers = attn_layers

        self.project_out = Linear(dim, dim_out) if exists(dim_out) else nn.Identity()
