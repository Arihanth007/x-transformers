...
class LevySMILES(Dataset):
    ...
    def __getitem__(self, idx: int) -> dict:
        ...
        # tokenize
        new_reactants = [self.start_token] + [self.char2idx[char] if char in self.char2idx.keys() else self.char2idx['?'] for char in atomwise_tokenizer(new_reactants)] + [self.end_token]
        new_products  = [self.start_token] + [self.char2idx[char] if char in self.char2idx.keys() else self.char2idx['?'] for char in atomwise_tokenizer(new_products)]  + [self.end_token]
    ...
    def collate_fn(self, batch):
        ...
        return {'encoder_input': encoder_input, 'decoder_input': encoder_output}
...
if __name__ == '__main__':
    ...
    train_dataset = LevySMILES(split='train')
    val_dataset   = LevySMILES(split='val')
    test_dataset  = LevySMILES(split='test')

    config['pad_token_id'] = train_dataset.char2idx['<PAD>']
    config['mask_token_id'] = train_dataset.char2idx['<MASK>']

    num_available_cpus = len(os.sched_getaffinity(0))
    num_workers = num_available_cpus // len(config['device_ids'])

    train_loader = DataLoader(train_dataset, batch_size=config['batch_size'], shuffle=True, num_workers=num_workers, collate_fn=train_dataset.collate_fn)
    val_loader   = DataLoader(val_dataset,   batch_size=config['batch_size'], shuffle=False, num_workers=num_workers, collate_fn=val_dataset.collate_fn)
    test_loader  = DataLoader(test_dataset,  batch_size=config['batch_size'], shuffle=False, num_workers=num_workers, collate_fn=test_dataset.collate_fn)

    batches_per_gpu = math.ceil(len(train_loader) / len(config['device_ids']))
    ...
    if config['train']:
        ...
        for (split, split_dm) in [('test', test_loader), ('val', val_loader)]:
            for ft in [0.999]:
